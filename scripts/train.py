...
from src.utils import setup_logging, check_gpu_memory, print_trainable_parameters
from src.data import load_conll2003_dataset, prepare_datasets, print_dataset_statistics
from src.model import setup_model, save_model_checkpoint
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Mistral-7B for Named Entity Recognition"
    )
    ...
    parser.add_argument(
        "--load-in-8bit",
        action="store_true",
        help="Load model in 8-bit (overrides config)"
    )
    parser.add_argument(
        "--no-load-in-8bit",
        dest="load_in_8bit",
        action="store_false",
        help="Don't load model in 8-bit"
    )
