...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with fine-tuned Mistral NER model"
    )
    ...
    parser.add_argument(
        "--file",
        type=str,
        help="File containing text to run inference on"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    for i in range(0, len(texts), batch_size):
        ...
        for j, text in enumerate(batch_texts):
            ...
            all_predictions.append({
                ...
                'words': words,
                'labels': word_labels,
                'entities': extract_entities(words, word_labels)
            })
...
def format_output(predictions: List[Dict[str, Any]], format_type: str = "inline") -> str:
    ...
    for pred in predictions:
        if format_type == "inline":
            ...
            tokens_with_labels = []
            for word, label in zip(pred['words'], pred['labels']):
                if label != 'O':
