...
def run_test(test_func, test_name):
    ...
    try:
        ...
        print("✗ FAILED")
        failed += 1
        errors.append({"test": test_name, "error": str(e), "type": "assertion"})
    except Exception as e:
        print("✗ ERROR")
        failed += 1
        errors.append({"test": test_name, "error": str(e), "type": "exception", "traceback": traceback.format_exc()})
...
def test_imports():
    ...
    modules = ["src.config", "src.utils", "src.data", "src.model", "src.evaluation", "src.training"]
...
def test_utils():
    ...
    from src.utils import detect_mixed_precision_support, estimate_memory_usage
