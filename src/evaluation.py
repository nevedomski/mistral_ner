...
def compute_metrics_factory(label_names: List[str], seqeval_metric=None):
    ...
    def compute_metrics(p: EvalPrediction) -> Dict[str, float]:
        ...
        if seqeval_metric is not None:
            ...
            metric_results = seqeval_metric.compute(
...
def save_predictions(
    ...
    with open(output_file, 'w') as f:
        for tok_seq, true_seq, pred_seq in zip(tokens, labels, predictions):
            for token, true_label, pred_label in zip(tok_seq, true_seq, pred_seq):
